...
#reroot species tree based on outgroup provided by user on the command line (must be a single species). Using ape functions to do this.

full_sptree_obj = read.tree(text=full_sptree)
full_sptree = write.tree(root(full_sptree_obj, args[2], resolve.root=TRUE))

sink(output_trees)
cat ("NJst tree based on full set of gene trees...\n", full_sptree, "\n\nNJst trees based on removal of one gene tree at a time...\n", sep="")
...
for (gt in 1:length(genetrees$tree)){
	...
	sub_sptree=NJst(genetrees$tree[-gt], genetrees$names, genetrees$names,species.structure)
	
	#root species tree based on outgroup provided by user on the command line (must be a single species). Using ape functions to do this.
	...
	sub_sptree = write.tree(root(sub_sptree_obj, args[2], resolve.root=TRUE))
	...
	for (node in 1:dim(full_sptree_nodes$nodes)[1]){
		...
		#skip the root node or terminals (all species or 1 species)
