...
our $gt_file;
our $gt_dir;
our $r_code1 = "pcbl_njst_code1.R";
our $r_code2 = "pcbl_njst_code2.R";
our $Rscript = "Rscript";
our $outgroup;
our $output;
our $threads = 1;
...
-e $r_code1 or die ("\nERROR: $r_code1 does not exist. Provide correct path to the provided R code for running NJst with --r_code1. \n\n$usage");
-e $r_code2 or die ("\nERROR: $r_code2 does not exist. Provide correct path to the provided R code for running NJst with --r_code2. \n\n$usage");
...
print STDERR "\n\n". (localtime) ."\nRunning PCBL using NJst as implemented in phybase\n\n";
my $gt_source;
if ($gt_file){$gt_source = $gt_file;}else{$gt_source = $gt_dir;}

print STDERR (localtime) . "\nAnalyzing gene trees in $gt_source using $outgroup as an outgroup to build full NJst species tree...\n\n";
...
#run NJst to generate species tree
...
print STDERR "\n". (localtime) . "\nGenerating PCBL data by rerunning NJst after excluding one gene tree at a time...\n\n\tCheck progress in $output\_trees file(s).\n\n\tNumber of specified threads: $threads\n\n";
...
##combine/format output files that were run in batches

my $FH_TREES = open_output ("$output\_trees.txt");
print $FH_TREES "NJst trees based on removal of one gene tree at a time...\n";
